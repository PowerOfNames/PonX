...
namespace Povox {
	...
	public:
		...
		virtual inline const void* GetCommandBuffer(uint32_t index) override { return GetFrame(index).Commands.Buffer; }
		...
		virtual void AddImGuiImage(float width, float height) override; //temp
	...
	private:
		...
		Scope<VulkanDescriptorAllocator> m_DescriptorAllocator = nullptr;
		Scope<VulkanDescriptorLayoutCache> m_DescriptorCache = nullptr;

		VkCommandBuffer m_ActiveCommandBuffer;
		Ref<VulkanRenderPass> m_ActiveRenderPass;
